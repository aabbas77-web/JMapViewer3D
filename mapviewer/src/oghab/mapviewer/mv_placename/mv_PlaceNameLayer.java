 ...
 */
public class mv_PlaceNameLayer extends AbstractLayer implements BulkRetrievable
{
    protected final mv_PlaceNameServiceSet placeNameServiceSet;
    ...
    protected List<mv_NavigationTile> navTiles = new ArrayList<mv_NavigationTile>();
     ...
     */
    public mv_PlaceNameLayer(mv_PlaceNameServiceSet placeNameServiceSet)
    {
        ...
        for (int i = 0; i < this.placeNameServiceSet.getServiceCount(); i++)
        {
            ...
            int calc1 = (int) (mv_PlaceNameService.TILING_SECTOR.getDeltaLatDegrees()
                / this.placeNameServiceSet.getService(i).getTileDelta().getLatitude().getDegrees());
            int numLevels = (int) Math.log(calc1);
            navTiles.add(
                new mv_NavigationTile(this.placeNameServiceSet.getService(i), mv_PlaceNameService.TILING_SECTOR, numLevels,
                    "top"));
        }
    ...
    public final mv_PlaceNameServiceSet getPlaceNameServiceSet()
    {
    ...
    protected class mv_NavigationTile
    {
        String id;
        protected mv_PlaceNameService placeNameService;
        public Sector navSector;
        protected List<mv_NavigationTile> subNavTiles = new ArrayList<mv_NavigationTile>();
        protected List<String> tileKeys = new ArrayList<String>();
        protected int level;

        mv_NavigationTile(mv_PlaceNameService placeNameService, Sector sector, int levels, String id)
        {
        ...
        protected void buildSubNavTiles()
        {
            if (level > 0)
            {
                ...
                {
                    subNavTiles.add(new mv_NavigationTile(placeNameService, subSectors[j], level - 1, this.id + "." + j));
                }
            }
        }

        public List<mv_NavigationTile> navTilesVisible(DrawContext dc, double minDistSquared, double maxDistSquared)
        {
            ArrayList<mv_NavigationTile> navList = new ArrayList<mv_NavigationTile>();
            if (this.isNavSectorVisible(dc, minDistSquared, maxDistSquared))
            {
                ...
                if (this.hasSubTiles())
                {
                    for (mv_NavigationTile nav : subNavTiles)
                    {
    ...
    {
        protected final mv_PlaceNameService placeNameService;
        ...
        protected mv_PlaceNameChunk dataChunk = null;

        Tile(mv_PlaceNameService placeNameService, Sector sector, int row, int column)
        {
            this.placeNameService = placeNameService;
            this.sector = sector;
            this.row = row;
        ...
        public void setDataChunk(mv_PlaceNameChunk chunk)
        {
            dataChunk = chunk;
        }

        public mv_PlaceNameChunk getDataChunk()
        {
            return dataChunk;
        }
        ...
        public mv_PlaceNameService getPlaceNameService()
        {
    ...
    protected Tile[] buildTiles(mv_PlaceNameService placeNameService, mv_NavigationTile navTile)
    {
    ...
    protected static class mv_PlaceNameChunk implements Cacheable
    {
        protected final mv_PlaceNameService placeNameService;
        ...
        protected mv_PlaceNameChunk(mv_PlaceNameService service, CharBuffer text, int[] textIndices,
        ...
        protected mv_PlaceNameService getPlaceNameService()
        {
    ...
    protected void doRender(DrawContext dc)
    {
        ...
        {
            mv_PlaceNameService placeNameService = this.placeNameServiceSet.getService(i);
            ...
            if (isSectorVisible(dc, placeNameService.getMaskingSector(), minDistSquared, maxDistSquared))
            {
                ...
                mv_NavigationTile navTile = this.navTiles.get(i);
                //drill down into tiles to find bottom level navTiles visible
                List<mv_NavigationTile> list = navTile.navTilesVisible(dc, minDistSquared, maxDistSquared);
                for (mv_NavigationTile nt : list)
                {
        ...
        double maxDisplayDistanceSquared)
    {
        ...
        {
            mv_PlaceNameChunk placeNameChunk = tile.getDataChunk();
    ...
    protected static boolean isServiceVisible(DrawContext dc, mv_PlaceNameService placeNameService)
    {
...
//        return placeNameService.getExtent(dc).intersects(dc.getView().getFrustumInModelCoordinates());
    ...
    protected static boolean isNameVisible(DrawContext dc, mv_PlaceNameService service, Position namePosition)
    {
    ...
    protected static class RequestTask implements Runnable, Comparable<RequestTask>
    {
        ...
        RequestTask(Tile tile, mv_PlaceNameLayer layer)
        {
            this.layer = layer;
            this.tile = tile;
        }
    ...
    protected static mv_PlaceNameChunk readTileData(Tile tile, java.net.URL url)
    {
...
//            java.io.BufferedInputStream buf = new java.io.BufferedInputStream(fis);//AliSoft
//            is = new java.util.zip.GZIPInputStream(buf);//AliSoft
...
//            javax.xml.parsers.SAXParserFactory.newInstance().newSAXParser().parse(is, handler);//AliSoft
            javax.xml.parsers.SAXParserFactory.newInstance().newSAXParser().parse(new File(path), handler);
//            javax.xml.parsers.SAXParserFactory.newInstance().newSAXParser().parse(sb.toString(), handler);
...
//        protected static final String TOPP_FULL_NAME_ND = "topp:full_name_nd";//AliSoft
        protected static final String TOPP_FULL_NAME_ND = "MapViewer:name";//AliSoft
//        protected static final String TOPP_LATITUDE = "topp:latitude";
//        protected static final String TOPP_LONGITUDE = "topp:longitude";
        protected static final String TOPP_COORDINATES = "gml:coordinates";
...
//        protected StringBuilder lonBuffer = new StringBuilder();
        protected StringBuilder LLBuffer = new StringBuilder();
        ...
        protected mv_PlaceNameChunk createPlaceNameChunk(mv_PlaceNameService service)
        {
            int numChars = this.textArray.length();
            CharBuffer textBuffer = newCharBuffer(numChars);
            textBuffer.put(this.textArray.toString());
            textBuffer.rewind();
            return new mv_PlaceNameChunk(service, textBuffer, this.textIndexArray, this.latlonArray, this.numEntries);
        }
        ...
        {
            String[] strPoints = this.LLBuffer.toString().split(" ");

            String[] strLLs = strPoints[0].split(",");
            
            double lat = this.parseDouble(strLLs[1]);
            double lon = this.parseDouble(strLLs[0]);
            ...
            this.LLBuffer.delete(0, this.LLBuffer.length());
...
//        protected void endEntry()
//        {
//            String strLL = this.LLBuffer.toString();
//            String[] strLLs = strLL.split(",");
//            
//            double lat = this.parseDouble(strLLs[1]);
//            double lon = this.parseDouble(strLLs[0]);
//            
...
//        }
        ...
        public void characters(char ch[], int start, int length)
        {
            ...
            if(TOPP_COORDINATES == internedTopQName)
                sb = this.LLBuffer;
...
//            retriever.setValue(URLRetriever.EXTRACT_ZIP_ENTRY, "true"); // supports legacy layers// AliSoft
            retriever.setValue(URLRetriever.EXTRACT_ZIP_ENTRY, "false"); // supports legacy layers// AliSoft
        }
        else
        {
    ...
    protected static class DownloadPostProcessor extends AbstractRetrievalPostProcessor
    {
        ...
        public DownloadPostProcessor(mv_PlaceNameLayer layer, Tile tile)
        {
            // No arg check; the class has protected access.
            this(layer, tile, null);
        }

        public DownloadPostProcessor(mv_PlaceNameLayer layer, Tile tile, FileStore fileStore)
        {
            // No arg check; the class has protected access.
    ...
    {
        mv_PlaceNameLayerBulkDownloader thread = new mv_PlaceNameLayerBulkDownloader(this, sector, resolution,
    ...
    public long getEstimatedMissingDataSize(Sector sector, double resolution, FileStore fileStore)
    {
        try
        {
            mv_PlaceNameLayerBulkDownloader downloader = new mv_PlaceNameLayerBulkDownloader(this, sector, resolution,
