 ...
 */
public class myMercatorTextureTile extends TextureTile
{
	private MercatorSector mercatorSector;

	public myMercatorTextureTile(MercatorSector mercatorSector, Level level,
			int row, int col)
	{
	...
	@Override
	public myMercatorTextureTile[] createSubTiles(Level nextLevel)
	{
		if (nextLevel == null)
		{
		...
		myMercatorTextureTile[] subTiles = new myMercatorTextureTile[4];

		TileKey key = new TileKey(nextLevelNum, 2 * row, 2 * col,
				nextLevelCacheName);
		myMercatorTextureTile subTile = this.getTileFromMemoryCache(key);
		if (subTile != null)
			subTiles[0] = subTile;
		else
			subTiles[0] = new myMercatorTextureTile(new MercatorSector(d0, d1,
		...
		if (subTile != null)
			...
			subTiles[1] = new myMercatorTextureTile(new MercatorSector(d0, d1,
		...
		if (subTile != null)
			...
			subTiles[2] = new myMercatorTextureTile(new MercatorSector(d1, d2,
		...
		if (subTile != null)
			...
			subTiles[3] = new myMercatorTextureTile(new MercatorSector(d1, d2,
					t1, t2), nextLevel, 2 * row + 1, 2 * col + 1);

		return subTiles;
	}

	protected myMercatorTextureTile getTileFromMemoryCache(TileKey tileKey)
	{
		return (myMercatorTextureTile) WorldWind.getMemoryCache(
				myMercatorTextureTile.class.getName()).getObject(tileKey);
	}
