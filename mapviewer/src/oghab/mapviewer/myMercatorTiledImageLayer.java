 ...
 */
public abstract class myMercatorTiledImageLayer extends AbstractLayer
{
    // Infrastructure
    private static final LevelComparer levelComparer = new LevelComparer();
    private LevelSet levels = null;
    private ArrayList<myMercatorTextureTile> topLevels;
    ...
    private ArrayList<myMercatorTextureTile> currentTiles = new ArrayList<myMercatorTextureTile>();
    private myMercatorTextureTile currentResourceTile;
    ...
    abstract protected void requestTexture(DrawContext dc, myMercatorTextureTile tile);

    abstract protected void forceTextureLoad(myMercatorTextureTile tile);

    public myMercatorTiledImageLayer()
    {

    }

    public void setLevelSet1(LevelSet levelSet)
    {
        if (levelSet == null)
        {
    ...
    private void createTopLevelTiles()
    {
        ...
        this.topLevels = new ArrayList<myMercatorTextureTile>(nLatTiles
            * nLonTiles);
        ...
        for (int row = firstRow; row <= lastRow; row++)
        {
            ...
            for (int col = firstCol; col <= lastCol; col++)
            {
                ...
                this.topLevels.add(new myMercatorTextureTile(new MercatorSector(
    ...
    {
        for (myMercatorTextureTile tile : this.topLevels)
        {
    ...
    private void addTileOrDescendants(DrawContext dc, myMercatorTextureTile tile)
    {
        ...
        myMercatorTextureTile ancestorResource = null;
        ...
        try
        {
            ...
            myMercatorTextureTile[] subTiles = tile.createSubTiles(this.levels
                .getLevel(tile.getLevelNumber() + 1));
            for (myMercatorTextureTile child : subTiles)
            {
    ...
    private void addTile(DrawContext dc, myMercatorTextureTile tile)
    {
    ...
    private void addTileToCurrent(myMercatorTextureTile tile)
    {
        this.currentTiles.add(tile);
    }

    private boolean isTileVisible(DrawContext dc, myMercatorTextureTile tile)
    {
    ...
    private boolean meetsRenderCriteria(DrawContext dc, myMercatorTextureTile tile)
    {
    ...
    private void draw(DrawContext dc)
    {
        ...
        {
            myMercatorTextureTile[] sortedTiles = new myMercatorTextureTile[this.currentTiles
                .size()];
    ...
    private static class LevelComparer implements
        Comparator<myMercatorTextureTile>
    {
        public int compare(myMercatorTextureTile ta, myMercatorTextureTile tb)
        {
    ...
    private void drawTileIDs(DrawContext dc,
        ArrayList<myMercatorTextureTile> tiles)
    {
        ...
        for (myMercatorTextureTile tile : tiles)
        {
    ...
    private BufferedImage requestImage(myMercatorTextureTile tile, String mimeType)
    ...
    private void downloadImage(final myMercatorTextureTile tile, String mimeType)
        throws Exception
    {
//        System.out.println("AliSoft:myMercatorTextureTile:downloadImage: "+tile.getPath());
        ...
        boolean abortOnError, BufferedImage image)
    {
        ...
        myMercatorTextureTile[][] tiles = this.getTilesInSector(sector,
            levelNumber);
        ...
        for (myMercatorTextureTile[] row : tiles)
        {
            for (myMercatorTextureTile tile : row)
            {
                if (tile == null)
                    continue;
    ...
    private myMercatorTextureTile[][] getTilesInSector(Sector sector,
        int levelNumber)
    {
        if (sector == null)
        {
        ...
        int numRows = nwRow - seRow + 1;
        int numCols = seCol - nwCol + 1;
        myMercatorTextureTile[][] sectorTiles = new myMercatorTextureTile[numRows][numCols];
        ...
        for (int row = nwRow; row >= seRow; row--)
        {
            for (int col = nwCol; col <= seCol; col++)
            {
                ...
                sectorTiles[nwRow - row][col - nwCol] = new myMercatorTextureTile(
    ...
    private BufferedImage getImage(myMercatorTextureTile tile, String mimeType)
        throws Exception
    {
        // Read the image from disk.
        BufferedImage image = this.requestImage(tile, mimeType);
//        System.out.println("AliSoft:myMercatorTextureTile:getImage: "+tile.getPath());
        if (image != null)
            return image;
    ...
    {
        private myMercatorTextureTile tile;

        public HttpRetrievalPostProcessor(myMercatorTextureTile tile)
        {
            this.tile = tile;
        }
        ...
        public ByteBuffer run(Retriever retriever)
        {
            ...
            if (htr.getResponseCode() == HttpURLConnection.HTTP_NO_CONTENT)
            {
                ...
                myMercatorTiledImageLayer.this.levels.markResourceAbsent(tile);
                return null;
            }
