...
public class StatusLayer extends AbstractLayer implements PositionListener, RenderingListener
{
    ...
    protected String stmXDisplay = "";
    protected String stmYDisplay = "";
    protected String stmZoneDisplay = "";
    ...
    protected int coordDecimalPlaces = 6;
    ...
    public StatusLayer()
    {
        ...
        Timer downloadTimer = new Timer(300, (java.awt.event.ActionEvent actionEvent) -> {
            ...
            {
//AliSoft                noNetwork = Logging.getMessage("term.NoNetwork");
                noNetwork = MainFrame.bundle.getString("M_NoNetwork");
                return;
            }
            else
                noNetwork = "";
        ...
        Timer netCheckTimer = new Timer(10000, (java.awt.event.ActionEvent actionEvent) -> {
            if (!showNetworkStatus)
                return;
...
//            String label = String.format(" %s   %s   %s   %s ", altDisplay, latDisplay, lonDisplay, elevDisplay);
            String label = String.format(" %s   %s   %s   %s   %s   %s   %s   %s ", "MapViewer", altDisplay, latDisplay, lonDisplay, elevDisplay, stmXDisplay, stmYDisplay, stmZoneDisplay);
    ...
    Position get_position_at(PositionEvent event)
    {
        Position newPos = event.getPosition();

//        Vec4 tempVec4 = eventSource.getView().getGlobe().computePointFromPosition(event.getPosition());
//        Position newPos = eventSource.getView().getGlobe().computePositionFromPoint(tempVec4);
        
//        Point p = event.getScreenPoint();
//        if(p == null)   p = new Point(0,0);
//        Vec4 screenCoords = new Vec4(p.x,p.y);
//        Vec4 cartesian = eventSource.getView().unProject(screenCoords);
//        Position newPos = eventSource.getView().getGlobe().computePositionFromPoint(cartesian);
    ...
    {
        Position newPos = get_position_at(event);
        if (newPos != null)
        {
            ...
            String[] res = mv_stm.Convert_Geo_To_XY_String(newPos.getLongitude().degrees, newPos.getLatitude().degrees);
            stmXDisplay = "X "+res[0];
            stmYDisplay = "Y "+res[1];
            stmZoneDisplay = "Zone "+res[2];
